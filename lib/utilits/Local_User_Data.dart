...
class globalAccountData {
  ...
  static Future setUserPin(int userPin) async =>
      await _preferences?.setInt(_userPin, userPin);

  static int? getUserPin() => _preferences?.getInt(_userPin);

  static Future setAdminPhone(String aminPhone) async =>
      await _preferences?.setString(_adminPhone, aminPhone);
  ...
  static Future setStateDialog(bool stateDialog) async =>
      await _preferences?.setBool(_stateDialog, stateDialog);

  static bool? getStateDialog() => _preferences?.getBool(_stateDialog);

  static Future setShowAgain(bool showAgain) async =>
      await _preferences?.setBool(_showAgain, showAgain);
  ...
  static Future setActiveStatus(String activeStatus) async =>
      await _preferences?.setString(_activeStatus, activeStatus);

  static String? getActiveStatus() => _preferences?.getString(_activeStatus);

  static Future setIsAdmin(bool isAdmin) async =>
      await _preferences?.setBool(_isAdmin, isAdmin);

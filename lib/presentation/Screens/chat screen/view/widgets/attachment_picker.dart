...
class AttachmentPicker extends StatefulWidget {
	final Function(File? file, Uint8List? fileBytes, String fileName, String type) onFileSelected;
		...
		try {
			if (kIsWeb) {
				...
				if (result != null && result.files.isNotEmpty) {
					...
					if (file.bytes != null) {
						widget.onFileSelected(null, file.bytes!, file.name, 'image');
					}
				}
			} else {
				...
				if (image != null) {
					...
					widget.onFileSelected(file, null, fileName, 'image');
				}
			}
		} catch (e) {
		...
		try {
			if (kIsWeb) {
				...
				if (result != null && result.files.isNotEmpty) {
					final file = result.files.single;
					if (file.bytes != null) {
						widget.onFileSelected(null, file.bytes!, file.name, 'video');
					}
				}
			} else {
				final ImagePicker picker = ImagePicker();
				...
				if (video != null) {
					...
					widget.onFileSelected(file, null, fileName, 'video');
				}
			}
		} catch (e) {
		...
		try {
			...
			if (result != null) {
				...
				if (kIsWeb) {
					...
					if (bytes != null) {
						widget.onFileSelected(null, bytes, fileName, type);
					}
				} else {
					final path = platformFile.path;
					if (path != null) {
						final file = File(path);
						widget.onFileSelected(file, null, fileName, type);
					}
				}
			}
		} catch (e) {

...
abstract class BaseGetBrandBySearchRemoteData {
  Future<BrandDataModel> getAllBrandBySearchFromRemote(
      {required String keyWard, required int page});
}

class GetBrandBySearchRemoteData extends BaseGetBrandBySearchRemoteData {
  @override
  Future<BrandDataModel> getAllBrandBySearchFromRemote(
      {required String keyWard, required int page}) async {
    print(keyWard);
    print(
        "${ApiConstant.baseUrl}${ApiConstant.firstSearchSecForBrand}${globalAccountData.getId().toString()}"
